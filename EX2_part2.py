...
# Function to Find the Best Split using Entropy
def best_split(X, y):
...
def load_data(file_path):
    ...
    # Separate features and target variable (assuming last column is the target variable)
    X = [[float(val) for val in row[:-1]] for row in data if row[-1] is not None]  # Excluding last column for X
    y = [int(row[-1]) for row in data if row[-1] is not None]  # Last column as y (target)
